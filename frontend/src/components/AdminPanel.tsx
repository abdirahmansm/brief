...
export default function AdminPanel({
  ...
  useEffect(() => {
    if (!open || !isAdmin || !normalizedEmail) return;
  ...
  }, [open, isAdmin, normalizedEmail, registeredAdmins]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
      <div className="w-full max-w-5xl overflow-hidden rounded-3xl border border-[rgba(139,92,246,0.24)] bg-[rgba(10,10,26,0.96)] shadow-[0_30px_90px_rgba(0,0,0,0.55)]">
        <div className="flex items-center justify-between border-b border-[rgba(139,92,246,0.16)] px-6 py-4">
          <div>
            <p className="text-sm font-semibold text-[var(--foreground)]">Owner Admin Console</p>
            <p className="text-xs text-[var(--muted-foreground)]">Operational insights for API health, report quality, and app usage.</p>
          </div>
          <button
            onClick={onClose}
          ...
          {!isAdmin ? (
            ...
            <div className="space-y-5">
              ...
              <section className="rounded-2xl border border-[rgba(139,92,246,0.16)] bg-[rgba(20,20,40,0.48)] p-4">
                ...
                <p className="mt-1 text-xs text-[var(--muted-foreground)]">Provider status, model routing defaults, and availability checks. Exact billing credits are tracked by provider dashboards.</p>
